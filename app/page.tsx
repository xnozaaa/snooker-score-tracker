...
export default function Home() {
  ...
  const savedDays = useMemo(() =>
    Object.entries(data.sessions)
      .map(([dateKey, savedSession]) => {
        const savedFrames = savedSession.frames.filter((frame) => frame.winner);
        const savedBreaks = [
          ...savedSession.frames.flatMap((frame) => frame.breaks),
          ...savedSession.manualHighBreaks,
        ];
        const savedFlukes = savedSession.frames.reduce(
          (total, frame) => total + frame.shots.length,
          0,
        );
        const savedFoulPoints = savedSession.frames.reduce(
          (total, frame) =>
            total + (frame.fouls ?? []).reduce((sum, entry) => sum + entry.points, 0),
          0,
        );
        return {
          dateKey,
          ...
          highBreak: savedBreaks.reduce((highest, entry) => Math.max(highest, entry.points), 0),
          flukes: savedFlukes,
          foulPoints: savedFoulPoints,
          hasEntries:
      ...
      .filter((savedDay) => savedDay.hasEntries)
  ...
  return (
    <main className="app-shell">
      ...
      <section className="stat-strip" aria-label="Today's summary">
        ...
        <a className="records-link" href="#past-games">Past games</a>
      </section>
          ...
          <section className="saved-days-panel reset-saved-days" aria-labelledby="saved-days-title">
            <div className="subsection-heading">
              <div>
                ...
                <h3 id="saved-days-title">Saved playing days</h3>
              </div>
